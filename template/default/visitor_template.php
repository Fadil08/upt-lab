...
<?php
// ===============================
// BACKGROUND VISITOR (WAJIB DI SINI)
// ===============================
?>
<?php if (!isset($_GET['p']) || $_GET['p'] !== 'visitor') : ?>
  <div class="<?= $sysconf['template']['classic_library_disableslide']
      ? 'vegas-slide c-header'
      : 'vegas-slide' ?>"
       style="position: fixed; z-index: -1">
  </div>
<?php endif; ?>
<div class="flex h-screen w-full visitor-page"
     id="visitor-counter"
     style="
       background: url('template/default/assets/images/visitor1.jpg')
...
<!--<div class="flex h-screen w-full visitor-page"
    ...
    <div class="bg-white w-full md:w-1/3 px-8 pt-8 pb-3 flex flex-col justify-between">
        <div>
			...
			<h3 class="font-light mb-1"><?= __('Welcome to '); ?></h3>
			<h3 class="font-light mb-3"><?= $sysconf['library_name']; ?></h3>
            <p class="lead">
                <?= __('Masukan Nomor ID (NIM) Anda.')?>
            </p>
            ...
            <form class="mt-4" @submit.prevent="onSubmit">
                <div class="form-group">
                    <label for="exampleInputEmail1"><?= __('ID (NIM) Anggota')?></label>
                    <input v-model="memberId" ref="memberId" autofocus type="text" class="form-control" id="exampleInputEmail1"
                           aria-describedby="emailHelp" placeholder="<?= __('Masukan Nomor ID (NIM) Anda')?>">
                </div>
                <div class="form-group">
                    ...
                    <small id="emailHelp" class="form-text text-muted"><?= __('Cukup masukkan ID (NIM) Saja jika Anda anggota dari ').$sysconf['library_name']; ?></small>
                </div>
                <button type="submit" class="btn btn-primary btn-block"><?= __('Check In')?></button> </br>
            
			</form>
			<a href="index.php"><button class="btn btn-primary btn-block"><?= __('Home')?></button></a>
        </div>						
        <div class="text-right">
            <small class="text-grey-dark"><?= __('Powered by ')?> <code>s4i</code></small>
        </div>
    </div>
        ...
        methods: {
            ...
            getQuotes: function() {
                ...
                axios.get('https://slims.web.id/kutipan/')
                    ...
                    .catch(() => {
                        this.quotes = {
                            content: "Terima Kasih Atas Partisipasinya dengan Mengisi Daftar Kehadiran.",
                            author: "Team Laboratorium"
                        }
                    })
                    .finally(() => {
                        this.textInfo = ''
