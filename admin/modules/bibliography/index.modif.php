...
if (!$in_pop_up) {
    ...
    <div class="menuBox">
        <div class="menuBoxInner biblioIcon">
            <div class="per_title">
                <h2><?php echo __('Katalog'); ?></h2>
            </div>
            <div class="sub_section">
                <div class="btn-group">
                    <a href="<?php echo MWB; ?>bibliography/index.php"
                       class="btn btn-default"><?php echo __('Daftar Katalog'); ?></a>
                    <a href="<?php echo MWB; ?>bibliography/index.php?action=detail"
                       class="btn btn-default"><?php echo __('Tambah Katalog Baru'); ?></a>
                </div>
                <form name="search" action="<?php echo MWB; ?>bibliography/index.php" id="search" method="get"
                    ...
                    <select name="field" class="form-control col-md-2">
                        ...
                        <option value="title"><?php echo __('Nama (Judul)'); ?> </option>
...
if (isset($_GET['action']) && $_GET['action'] == 'history') {
    ...
    /* Form Element(s) */
    // biblio title
    $form->addTextField('textarea', 'title', __('Nama (Judul)') . '*', $rec_d['title'] ?? '', 'rows="1" class="form-control"',
    ...
    $str_input .= '<iframe name="authorIframe" id="authorIframe" class="form-control auto-resize-iframe" style="width: 100%; min-height: 80px; max-height: 300px;" src="' . MWB . 'bibliography/iframe_author.php?biblioID=' . ($rec_d['biblio_id'] ?? '') . '&block=0"></iframe>';
    ...
    echo '<style>#simbioFormRowsor{display:none!important}</style>';
	// end of modification
	...
	echo '<style>#simbioFormRowedition{display:none!important}</style>';
  ...
  <div class="bnt btn-group"><div class="btn btn-info" data-toggle="collapse" data-target="#batchItemDetail" aria-expanded="false" aria-controls="batchItemDetail">' . __('Options') . '</div>';
    ...
    echo '<style>#simbioFormRowisbn_issn{display:none!important}</style>';
	// biblio publisher
    ...
    echo '<style>#simbioFormRowpublisherID{display:none!important}</style>';
	// biblio publish year
    ...
    echo '<style>#simbioFormRowplaceID{display:none!important}</style>';
	// biblio collation
    ...
    echo '<style>#simbioFormRowseriesTitle{display:none!important}</style>';
    ...
    echo '<style>#simbioFormRowclass{display:none!important}</style>';
	// biblio call_number
	...
	echo '<style>
	tr[data-row-index="24"] td {
		display: none !important;
	}
	tr[data-row-index="25"] td {
		display: none !important;
	}
	</style>';
    /**
    ...
    if ($sysconf['index']['type'] == 'index' || $sysconf['index']['type'] == 'sphinx') {
        ...
        // table spec
        //$table_spec = 'search_biblio AS `index` LEFT JOIN item ON `index`.biblio_id=item.biblio_id'; // script Asli >> script tambahan line 1291-1293
        $table_spec = 'search_biblio AS `index`
		LEFT JOIN item ON `index`.biblio_id = item.biblio_id
		LEFT JOIN mst_location ON item.location_id = mst_location.location_id';
        ...
        if ($can_read AND $can_write) {
            $datagrid->setSQLColumn('index.biblio_id', 'index.title AS \'' . __('Title') . '\'', 'index.labels', 'index.image',
                'index.author',
                //'index.isbn_issn AS \'' . __('ISBN/ISSN') . '\'', // script Asli
        ...
        // table spec
        //$table_spec = 'biblio LEFT JOIN item ON biblio.biblio_id=item.biblio_id'; // script Asli >> script tambahan line 1322-1324
        $table_spec = 'search_biblio AS `index`
		LEFT JOIN item ON `index`.biblio_id = item.biblio_id
		LEFT JOIN mst_location ON item.location_id = mst_location.location_id';
        ...
        if ($can_read AND $can_write) {
            $datagrid->setSQLColumn('biblio.biblio_id', 'biblio.biblio_id AS bid',
                ...
                //'biblio.isbn_issn AS \'' . __('ISBN/ISSN') . '\'', // script Asli
				'mst_location.location_name AS \'' . __('Lokasi') . '\'', // script tambahan
                'IF(COUNT(item.item_id)>0, COUNT(item.item_id), \'<strong style="color: #f00;">' . __('None') . '</strong>\') AS \'' . __('Copies') . '\'',
